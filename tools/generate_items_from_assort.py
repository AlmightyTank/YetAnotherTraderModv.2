...
Force every row to cash-only while still preserving the original barter recipe in BarterScheme:
...
def generate_items(
    ...
    for root in sellable_roots:
        ...
        cash_only = force_cash_only_rows or is_cash_only_scheme(normalized_scheme)

        output.append(
            {
                "OfferId": offer_id,
                "TplId": sold_tpl,
                "ItemName": resolve_name(sold_tpl, tarkov_dev_names, locale_names, catalog_names),
...
def build_report(out_path: Path, output: list[dict[str, Any]], warnings: list[str]) -> str:
    cash_rows = sum(1 for row in output if row.get("CashOnly") is True)
    barter_rows = len(output) - cash_rows
    ...
    lines = [
        ...
        f"CashOnly rows: {cash_rows}",
        f"Barter-capable rows: {barter_rows}",
